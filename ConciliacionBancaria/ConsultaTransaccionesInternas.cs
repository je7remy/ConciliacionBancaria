...
{
    public partial class ConsultaTransaccionesInternas : Form
    {
        ...
        public ConsultaTransaccionesInternas()
        {
            InitializeComponent();
        ...
        private void BusquedaBancos_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.TransaccionID = 0; //variable global que tomará el valor seleccionado
        ...
        {
            if (MessageBox.Show("¿Estás seguro de que deseas cerrar la Consulta Transacciones Internas?", "Cerrar Consulta Transacciones Internas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                e.Cancel = true;
            }
        }
        ...
        private void BAceptar_Click(object sender, EventArgs e)
        {
            ...
            //    Program.TransaccionID = Convert.ToInt32(DGVDatos.CurrentRow.Cells[0].Value);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Tbuscar.Text.Trim())) // Si se introdujo un dato en el textbox
            {
                ...
                if (int.TryParse(Tbuscar.Text.Trim(), out int TransaccionID))
                {
                    valorparametro = Tbuscar.Text.Trim();
                    MostrarDatos1(TransaccionID, null); // Pasar null para indicar que no se busca por Tipo
                }
        ...
        private void MostrarDatos1(int? TransaccionID, string Tipo)
        {
            DataTable dt = CNTransaccionesInternas.ObtenerTransaccionInternaPorID(TransaccionID, Tipo);
        ...
        private void MostrarDatos()
        {
            ...
            DataTable dt = CNTransaccionesInternas.ObtenerTransaccion(); // Acceder al método estático
