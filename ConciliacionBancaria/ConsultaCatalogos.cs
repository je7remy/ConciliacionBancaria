...
{
    public partial class ConsultaCatalogos : Form
    {
        ...
        private void BusquedaBancos_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.CatalogoID = 0; //variable global que tomará el valor seleccionado
        ...
        {
            if (MessageBox.Show("¿Estás seguro de que deseas cerrar la Consulta Catalogos?", "Cerrar Consulta Catalogos", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                e.Cancel = true;
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Tbuscar.Text.Trim())) // Si se introdujo un dato en el textbox
            {
                ...
                if (int.TryParse(Tbuscar.Text.Trim(), out int CatalogoID))
                {
                    valorparametro = Tbuscar.Text.Trim();
                    MostrarDatos1(CatalogoID, null); // Pasar null para indicar que no se busca por Nombre
                }
        ...
        private void MostrarDatos1(int? catalogo, string nombre)
        {
            DataTable dt = CNCatalogos.ObtenerCatalogoPorID(catalogo, nombre);
            ...
            {
                MessageBox.Show("No se encontraron catalogos."); // Mensaje si no se encontraron resultados
            }
        }
        ...
        private void MostrarDatos()
        {
            ...
            DataTable dt = CNCatalogos.ObtenerCatalogo(); // Acceder al método estático
