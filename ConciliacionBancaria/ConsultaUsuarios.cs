...
namespace ConciliacionBancaria
{
    public partial class ConsultaUsuarios : Form
    {
        ...
        private void BusquedaBancos_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.UsuarioID = 0; //variable global que tomará el valor seleccionado
        ...
        {
            if (MessageBox.Show("¿Estás seguro de que deseas cerrar la Consulta Usuarios?", "Cerrar Consulta Usuarios", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                e.Cancel = true;
            }
        }
        ...
        private void BAceptar_Click(object sender, EventArgs e)
        {
            ...
            //    Program.UsuarioID = Convert.ToInt32(DGVDatos.CurrentRow.Cells[0].Value);
     ...
     private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Tbuscar.Text.Trim())) // Si se introdujo un dato en el textbox
            {
                ...
                if (int.TryParse(Tbuscar.Text.Trim(), out int UsuarioID))
                {
                    valorparametro = Tbuscar.Text.Trim();
                    MostrarDatos1(UsuarioID, null); // Pasar null para indicar que no se busca por NombreUsuario
                }
                else // Si no es un número, se asume que es el nombre del usuario
                {
                    valorparametro = Tbuscar.Text.Trim();
        ...
        private void MostrarDatos1(int? UsuarioID, string NombreUsuario)
        {
            DataTable dt = CNUsuarios.ObtenerUsuarioPorID(UsuarioID, NombreUsuario);
            ...
            {
                MessageBox.Show("No se encontraron usuarios."); // Mensaje si no se encontraron resultados
            }
        }
        ...
        private void MostrarDatos()
        {
            ...
            DataTable dt = CNUsuarios.ObtenerUsuario(); // Acceder al método estático
