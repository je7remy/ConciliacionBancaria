...
{
    public partial class BusquedaCuentasBancarias : Form
    {
        ...
        public BusquedaCuentasBancarias()
        {
            InitializeComponent();
        ...
        private void BusquedaBancos_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.CuentaID = 0; //variable global que tomará el valor seleccionado
        ...
        {
            if (MessageBox.Show("¿Estás seguro de que deseas cerrar las Cuentas Bancarias?", "Cerrar Bancos", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                e.Cancel = true;
            }
        }
        ...
        private void BAceptar_Click(object sender, EventArgs e)
        {
            if (DGVDatos.CurrentRow != null) //Si el DataGridView no está vacío
            {
                ...
                Program.CuentaID = Convert.ToInt32(DGVDatos.CurrentRow.Cells[0].Value);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Tbuscar.Text.Trim())) // Si se introdujo un dato en el textbox
            {
                ...
                if (int.TryParse(Tbuscar.Text.Trim(), out int cuenta))
                {
                    valorparametro = Tbuscar.Text.Trim();
                    MostrarDatos1(cuenta);
                }
        ...
        private void MostrarDatos1(int? cuenta)
        {
            DataTable dt = CNCuentasBancarias.ObtenerCuentaBancariaPorID(cuenta, valorparametro);
        ...
        private void MostrarDatos()
        {
            ...
            DataTable dt = CNCuentasBancarias.ObtenerCuenta(); // Acceder al método estático
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                DGVDatos.Columns[6].Width = 40; 
                DGVDatos.Columns[7].Width = 80; 
                DGVDatos.Columns[8].Width = 60; 
                DGVDatos.Columns[9].Width = 90;
                DGVDatos.Columns[10].Width = 60;
                DGVDatos.Columns[11].Width = 90;
            }
            else
            {
